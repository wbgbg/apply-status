    ...
    function refresh() {
        ...
        $.getJSON("/json/record.json", function(result) {
            for (var i = 0; i < result.list.length; i++) {
                var node = "<tr class = \"recordList\"><th>" + result.list[i].company + "</th><th class=\"hidden-xs\">" + 
                result.list[i].position + "</th><th>" + result.list[i].status + "</th><th class=\"hidden-xs\">" + 
                result.list[i].comment + "</th>" + "<th><button type=\"button\" class=\"btn btn-primary modifyList aria-hidden\"> <span class=\"glyphicon glyphicon-wrench\" aria-hidden=\"true\"> </span> modify </button> &nbsp;" + 
